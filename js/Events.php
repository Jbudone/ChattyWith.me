...
echo <<<EOF
	...
	var hk_event_unknown_command=function(){ console.log("What is wrong with you ??   "+arguments[1]); };
		hk_event_parsed_bad_format=function(){ console.log(arguments[1]); },
		hk_event_request_from_undefined_event=function(){ console.log("Now why would you go requesting an undefined object? silly silly -- "+(typeof this.evtref)); },
		hk_event_request_exception_thrown=function(){ console.log("Some exception thrown: "); console.log(arguments); },
		...
		hk_server_event_append_message=function(){ 
			// NOTE: Append ALL messages together, each with a chanid="[id here]", then edit CSS to display:none to all EXCEPT the current chanid
			// 		< span > < span datetime > < span username > says: < span message > < / span >  use  span.datetime.minwidth or span.datetime.display to further control these
	...
	var Events={
		...
		ServerEvent: {
			...
			ESRV_LEAVE:{				_eventid:ESRV_LEAVE,
										parseFormat:['suserid','susernick'],
										message:'%susernick as left #%channame',
			...
			ESRV_SETOPS_CHANOPS_OFF:{	_eventid:ESRV_SETOPS_CHANOPS_OFF,
										parseFormat:['suserid','susernick','duserid','dusernick'],
										message:'%susernick has has stripped %dusernick of voice operator status -v',
										handler:client.hsrv_modify_ops },
